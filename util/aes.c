/*
Downloaded from here: https://github.com/kokke/tiny-AES128-C
...
/*
 * simple re-defines do use the existing code with a context
 */
#define state		ctx->state
#define RoundKey	ctx->RoundKey
#define Iv			ctx->InitialVector
#define KEYLEN		AES128_KEYLEN	
...
static void AddRoundKey(AES_CTX *ctx, uint8_t round)
{
  uint8_t i,j;
  for(i=0;i<4;++i)
  {
...
static void MixColumns(AES_CTX *ctx)
{
  uint8_t i;
  uint8_t Tmp,Tm,t;
  for(i = 0; i < 4; ++i)
...
static void InvMixColumns(AES_CTX *ctx)
{
  int i;
  uint8_t a,b,c,d;
  for(i=0;i<4;++i)
...
static void Cipher(AES_CTX *ctx)
{
  uint8_t round = 0;
...
static void InvCipher(AES_CTX *ctx)
{
  uint8_t round=0;
...
void AES128_InitContext(AES_CTX *ctx, const uint8_t* key, const uint8_t* initialVector)
{
    ...
    if (initialVector)
        memcpy(ctx->InitialVector, initialVector, KEYLEN);
...
static void XorWithIv(AES_CTX *ctx, uint8_t* buf)
{
    if (sizeof(int) == 4) {
        uint32_t *data = (uint32_t *)buf;
        uint32_t *iVec = (uint32_t *)Iv;
...
void AES128_CBC_encrypt_buffer(AES_CTX *ctx, uint8_t* output, const uint8_t* input, uint32_t length)
{
  ...
  for(i = 0; i < length; i += KEYLEN)
  {
    ...
    state = (state_t*)output;
    Cipher(ctx);
    memcpy(ctx->InitialVector, output, KEYLEN);
    input += KEYLEN;
    output += KEYLEN;
  }
...
void AES128_CBC_decrypt_buffer(AES_CTX *ctx, uint8_t* output, const uint8_t* input, uint32_t length)
{
  ...
  for(i = 0; i < length; i += KEYLEN)
  {
    ...
    InvCipher(ctx);
    XorWithIv(ctx, output);
    memcpy(ctx->InitialVector, input, KEYLEN);
    input += KEYLEN;
    output += KEYLEN;
  }
